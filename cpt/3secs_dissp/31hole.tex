...
\begin{figure}[!b]
  ...
  matrix~\cite{Lindblad-76,GKS-76,BP-book,RH-book,DR-21}.  To
  ...
  Refs.~\cite{FCKD-19,LHFHCE-19,KMS-19,WSDK-20,FMKCD-20,AC-21}, mainly
  ...
  dissipation.
  \rev{Note that reflection symmetry with respect to
...
\centering
    ...
    \caption{Asymptotic stationary limit of the particle density $n_x$
      ...
      small spatial oscillations, \rev{which appear similar to the
...
\centering
    ...
    \caption{The Liouvillian gap $\Delta_{\cal L}$ for particle-decay
      ...
      system size $L=2M+1$. \rev{The curves appear to converge with
...
\centering
  ...
  slower than that for $w=1$.  \rev{The suppression in the limit of
    strong dissipation, for large $w$, may be interpreted as a
    quantum-Zeno-like phenomenon~\cite{MS-77,FP-08}, where a strong
  ...
  $w$. \rev{The corresponding values $w_{\rm max}$ and $L^3 \Delta_{\cal
...
\centering
  ...
  \rev{We want to check whether the
...
\centering
  ...
  \rev{Again we want to check whether the out-of-equilibrium dynamics in
...
\centering
  ...
  (\ref{EQLindblad}) and (\ref{Lindop}). \rev{As already mentioned in
